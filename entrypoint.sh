...
[program:graph-node]
command=bash -c 'sleep 15 && graph-node --postgres-url %(ENV_DATABASE_URL)s --ethereum-rpc %(ENV_ETHEREUM_RPC)s --ipfs %(ENV_IPFS_URL)s --http-port 8000 --ws-port 8001 --admin-port 8020 --index-node-port 8030 --metrics-port 8040'
directory=/app
autostart=true
autorestart=true
